...
function CustomersPage() {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="display-title text-3xl">Customer Database</h1><p className="text-sm text-muted-foreground mt-1">{filtered.length} profiles · live company records</p></div><div className="flex gap-2"><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name, email, phone…" aria-label="Search customers" className="w-full sm:w-80 bg-panel2" /><Button onClick={openCreate}><Plus className="size-4" /> Add customer</Button></div></div>
